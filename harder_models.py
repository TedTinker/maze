...
spe_size = 1 ; action_size = 2 ; obs_num = 2
...
class Forward(nn.Module): # PVRNN!
    ...
    def forward(self, rgbd, spe, prev_action, hq_m1_list = None):
        ...
        for layer in range(self.layers):
            ...
            if(layer == 0):
                ...
                zq_mu, zq_std = var(torch.cat((relu_hq_m1, obs),     dim=-1), self.zq_mu_layers[layer], self.zq_std_layers[layer], self.args)        
            else:
                zp_mu, zp_std = var(relu_hq_m1, self.zp_mu_layers[layer],     self.zp_std_layers[layer], self.args)
    ...
    def predict(self, action, z_mus, z_stds, h_m1_list, quantity = 1):
        ...
        h_m1 = h_m1_list[0] ; h_m1_up = h_m1_list[1]
        h, _ = self.mtrnn_layers[0](torch.cat([z_mu, h_m1_up], dim = -1), h_m1)       
        ...
        for _ in range(quantity):
            z = sample(z_mu, z_std)
            h, _ = self.mtrnn_layers[00](torch.cat([z, h_m1_up], dim = -1), h_m1)   
